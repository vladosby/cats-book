...
final case class Cat3(name: String, age: Int, color: String)
...
object ShowCatsMain extends App {
  ...
  println(Cat2(name = "Cat name", age = 12, color = "blue").show)
}
...
object ShowInstances {
  ...
  implicit val catShow: Show[Cat2] = Show.show(cat => s"${cat.name} is a ${cat.age} year-old ${cat.color} cat.")
